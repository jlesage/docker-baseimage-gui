...
@test "Checking availability of HTTPs port 5800..." {
    TIMEOUT=30
    while (( TIMEOUT > 0 )); do
        run curl --silent --insecure --connect-timeout 2 https://127.0.0.1:5802
        [ "$status" -eq 0 ] && break
        sleep 1
        (( TIMEOUT-- ))
    done
...
@test "Checking availability of VNC SSL port 5900..." {
    TIMEOUT=30
    while (( TIMEOUT > 0 )); do
        run no_stderr timeout 2 openssl s_client -quiet -verify_quiet -connect 127.0.0.1:5902 2>/dev/null
