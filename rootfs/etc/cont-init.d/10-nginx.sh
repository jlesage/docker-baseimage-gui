...
if [ "${CAC_AUTH:-0}" -eq 1 ]; then
    cp /defaults/DoD_CAs.pem /config/certs/DoD_CAs.pem
    sed-patch '19i \\tssl_client_certificate /config/certs/DoD_CAs.pem; \n\tssl_verify_client on; \n\tif ($ssl_client_s_dn !~ "CN=CAC_Common_Name") { \n\t\treturn 403; \n\t} \n' /etc/nginx/default_site.conf
    sed-patch "s/CAC_Common_Name/$CAC_Common_Name/" /etc/nginx/default_site.conf
fi
